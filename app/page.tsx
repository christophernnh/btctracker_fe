    ...
    eventSource.onmessage = (event) => {
      ...
      setStats((prevStats) => {
        ...
        return {
          ...
          cumulative_volume_usd: (oldVolume + newEvent.total_value_usd).toFixed(2),
  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-neutral-200 font-mono p-6 sm:p-12 selection:bg-neutral-800">
      ...
      <header className="border-b border-neutral-900 pb-6 mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight uppercase text-neutral-100">BTC Volume Tracker</h1>
          <p className="text-xs text-neutral-500 mt-1">Real-time distributed algorithmic event sourcing ledger.</p>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-neutral-900/30 border border-neutral-900 rounded p-5">
          <span className="text-[10px] uppercase tracking-wider text-neutral-500 block mb-1">Asset Token</span>
          <span className="text-lg font-bold text-neutral-100">BTC / USDT</span>
        </div>
        <div className="bg-neutral-900/30 border border-neutral-900 rounded p-5">
          <span className="text-[10px] uppercase tracking-wider text-neutral-500 block mb-1">Last Execution Price</span>
          <span className="text-lg font-bold text-emerald-400">
            {stats ? `$${parseFloat(stats.last_price).toLocaleString(undefined, { minimumFractionDigits: 2 })}` : "Warming up..."}
          </span>
        </div>
        <div className="bg-neutral-900/30 border border-neutral-900 rounded p-5">
          <span className="text-[10px] uppercase tracking-wider text-neutral-500 block mb-1">Global Transacted Volume</span>
          <span className="text-lg font-bold text-neutral-300">
            {stats ? `$${parseFloat(stats.cumulative_volume_usd).toLocaleString(undefined, { maximumFractionDigits: 0 })}` : "Calibrating..."}
          </span>
        </div>
      </section>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="text-neutral-500 border-b border-neutral-900">
                  ...
                  <th className="pb-3 font-medium text-right">Aggregated Volume (USD)</th>
                </tr>
              </thead>
              <tbody>
                {leaderboard.map((item, index) => (
                  <tr key={item.trade_id} className="border-b border-neutral-900/50 hover:bg-neutral-900/10 transition-colors">
                    <td className="py-3 text-neutral-600 font-bold">{(index + 1).toString().padStart(2, "0")}</td>
                ...
                {leaderboard.length === 0 && (
                  <tr>
                    <td colSpan={3} className="py-8 text-center text-neutral-600">Waiting for data blocks to ingest...</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="flex-1 space-y-2 max-h-[420px] overflow-y-auto pr-2 scrollbar-thin">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <span className={isWhale ? "text-amber-500" : "text-neutral-600"}>
                      {isWhale ? "⚡ [WHALE]" : "• [TRADE]"}
                    </span>
                    <span className="text-neutral-300 font-medium">{log.account_id}</span>
                  </div>
                  <div className="text-right">
                    <span className={isWhale ? "text-amber-400" : "text-neutral-200"}>
                      {log.quantity.toFixed(4)} BTC
                    </span>
                    <span className="mx-2 text-neutral-600">@</span>
                    <span>${log.price.toFixed(2)}</span>
                    <span className={`ml-3 inline-block min-w-[80px] text-right ${isWhale ? "text-amber-400" : "text-neutral-500"}`}>
            ...
            {liveLog.length === 0 && (
              <div className="text-center text-neutral-600 py-16">
                Listening to <code className="bg-neutral-900 px-1 py-0.5 rounded text-neutral-500">channel:market_events</code>...
              </div>
            )}
          </div>
        </section>
